# Генерація автомобільного знака. Дві букви, чотири цифри, дві букви.
#
# Останні дві літери зі списку: A, B, C, E, H, I, K, M, O, P, T, X (використовуються українські літери, що мають графічні
# відповідники в латиниці)
import random
...
def car_number_generator():
    ...
    random_code = random.choice(list(data_transliterated[random_year].keys()))
    region_value = data_transliterated[random_year][random_code]

    numbers = str(random.randint(1000, 9999))
    initials = ''.join(random.choices(set_of_letters, k=2))
    car_number = ''.join([random_code, numbers, initials])
