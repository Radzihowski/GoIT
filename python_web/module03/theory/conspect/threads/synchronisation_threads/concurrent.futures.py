...
def greeting(name):
    logging.debug(f'string for: {name}')
    sleep(randint(0, 3))
    return f'Hello {name}'
...
# Пул можна створити в менеджері контексту як у прикладі, щоб бути впевненим, що всі ресурси будуть коректно повернуті
# до системи після завершення. Але це не обов'язково, можна створити пул і потім викликати у нього метод shutdown,
...
# Зверніть увагу, що код паралельно виконується не більше ніж двома потоками — ThreadPoolExecutor-0_0 та
# ThreadPoolExecutor-0_1. Результат повертається лише після того, як усі вхідні дані оброблені.